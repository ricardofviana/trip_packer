...
class Trip:
    ...
    @property
    def packing_list(self) -> list["Packing"]:
...
class Packing:
    ...
    bag_id: Mapped[int] = mapped_column(ForeignKey("bags.id"), nullable=True)
    quantity: Mapped[int] = mapped_column(default=1)
    status: Mapped[ItemStatus] = mapped_column(default=ItemStatus.UNPACKED)
    created_at: Mapped[datetime] = mapped_column(init=False, server_default=func.now())
    updated_at: Mapped[datetime] = mapped_column(init=False, server_default=func.now(), onupdate=func.now())

    __table_args__ = (UniqueConstraint("trip_id", "item_id", name="_trip_item_uc"),)
