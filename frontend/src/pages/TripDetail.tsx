...
export default function TripDetailPage() {
  ...
  const overview = useMemo(() => {
    // TODO: Implement overview calculation based on items in bags or fetch from backend if available
    return undefined;
  }, [tripId, bags]);
...
function BagColumn({ bag, filter, allBags, onChange }: { bag: Bag; filter: ItemStatus | "ALL"; allBags: Bag[]; onChange: () => void }) {
  ...
  const addItem = async () => {
    const n = name.trim();
    if (!n) return;
    await packingRepo.addLuggageItem(bag.id, { item_id: "", quantity: qty || 1 }); // item_id will be generated by backend
    setName("");
    setQty(1);
    setStatus("UNPACKED");
    refresh();
